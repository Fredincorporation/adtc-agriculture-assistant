...
class ContextGuardrail:
    def __init__(self):
        ...
        # Keywords that trigger context requirements
        self.location_intents = ["plant", "sow", "planting", "season", "period", "rain", "when to"]
        self.crop_intents = ["fertilizer", "pest", "spacing", "disease", "yield", "harvest"]

    def extract_known_entities(self, query: str):
        """Extracts location or crop if mentioned in the query text."""
        query_lower = query.lower()
        
        # Sample common African locations/regions
        ...
        # Sample crops
        crops = ["maize", "cassava", "cowpea", "yam", "teff", "rice", "groundnut", "tomato", "coffee", "cocoa"]
        for crop in crops:
            if crop in query_lower:
                self.session_context["crop"] = crop.title()

    def check_and_enrich(self, raw_query: str) -> str:
        """Evaluates missing slots and prompts user if context is required."""
        self.extract_known_entities(raw_query)
        query_lower = raw_query.lower()
        
        # 1. Check for Missing Location in Seasonal / Planting Queries
        ...
        # Build Enriched Query for RAG Retrieval
